...
import { FileText, Filter, Calendar, MapPin, Star, RotateCcw, CheckCircle2, ShieldAlert, ArrowRight, Eye, X } from 'lucide-react';
...
export default function MyComplaintsPage() {
  ...
  const handleReopenSubmit = (e) => {
    ...
    alert(`Complaint ${selectedComplaint.complaintId} has been reopened. Status changed to Reopened.`);
        ...
        <main className="flex-1 p-4 lg:p-8 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">Real-time status updates, resolution proofs, and reopening requests.</p>
            </div>
            ...
            {filtered.map(item => {
              ...
              return (
                <div
                  ...
                  className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between space-y-4 hover:border-slate-700 transition-colors shadow-lg"
                >
          ...
          {selectedComplaint && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in overflow-y-auto">
              <div className="bg-slate-900 border border-slate-800 w-full max-w-3xl rounded-3xl shadow-2xl p-6 space-y-6 my-8 max-h-[90vh] overflow-y-auto">
                ...
                {/* Reopen Action for Resolved complaints */}
                ...
                {/* Citizen Rating form for Resolved issues */}
                {selectedComplaint.status === 'Resolved' && (
                  <form onSubmit={handleFeedbackSubmit} className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-3">
                    <h4 className="text-xs font-bold text-slate-300">Rate Resolution Quality</h4>
          ...
          {showReopenModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm">
              <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl p-6 space-y-4">
                <h3 className="text-sm font-bold text-white">Reopen Complaint Reason</h3>
                <textarea
                  required
                  rows={3}
                ...
                <div className="flex gap-2 text-xs">
                  <button onClick={() => setShowReopenModal(false)} className="flex-1 py-2 bg-slate-800 text-slate-300 font-semibold rounded-xl">
                    Cancel
                  </button>
